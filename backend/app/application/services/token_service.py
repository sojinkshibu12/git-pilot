...
class TokenService:
    ...
    async def decrypt_credential(
        self, github_account_id: uuid.UUID
    ) -> DecryptedCredential:
        ...
        if cred.refresh_expires_at and cred.refresh_expires_at < now:
        ...
        try:
            ...
            refresh = self._vault.decrypt(cred.refresh_token_encrypted) if cred.refresh_token_encrypted else None
    ...
    async def access_token_for_user(self, user_id: uuid.UUID) -> str:
        ...
        account = await self._session.scalar(
            select(GitHubAccount).where(
                GitHubAccount.user_id == user_id, GitHubAccount.deleted_at.is_(None)
            ).limit(1)
        )
        if account is None:
