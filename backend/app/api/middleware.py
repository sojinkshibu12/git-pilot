...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: object) -> Response:
        ...
        if self._settings.is_production:
            response.headers.setdefault(
                "Strict-Transport-Security", f"max-age={self._settings.HSTS_MAX_AGE}; includeSubDomains"
            )
...
class CSRFCheckMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: object, settings: Settings, cors_origins: list[str]) -> None:
        super().__init__(app)
        self._settings = settings
    ...
    async def dispatch(self, request: Request, call_next: object) -> Response:
        ...
        if origin:
            if origin not in self._origins and not origin.startswith(request.base_url.scheme + "://" + request.base_url.hostname):
                logger.warning("csrf_origin_rejected", origin=origin, path=request.url.path)
                return Response(status_code=403, content='{"detail":"Origin rejected","code":"csrf_origin","status":403}')
        elif referer:
            from urllib.parse import urlparse

            host = urlparse(referer).hostname
            if host and host not in {request.base_url.hostname} | {urlparse(o).hostname for o in self._origins}:
                logger.warning("csrf_referer_rejected", referer=referer, path=request.url.path)
                return Response(status_code=403, content='{"detail":"Referer rejected","code":"csrf_referer","status":403}')
        return await call_next(request)
...
class SecurityLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: object) -> Response:
        ...
        response = await call_next(request)
        duration_ms = round((__import__("time").perf_counter() - start) * 1000, 2)
        logger.info(
            "http_request",
